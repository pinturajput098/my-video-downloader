...
# Ultra-Premium UI with clear User Ad Navigation Instructions
HTML_UI = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="text-slate-100 min-h-screen font-sans flex flex-col justify-between">
    ...
    <main class="container mx-auto px-4 py-12 flex-grow flex flex-col items-center justify-center max-w-2xl">
        ...
        <div class="w-full glass-card rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
            ...
            <div id="loader" class="hidden mt-8 text-center py-6">
                ...
                <p class="text-slate-400 text-xs animate-pulse">Routing via backup mirror cluster...</p>
            </div>
            ...
            <!-- RESULT CARD WITH SMART USER NOTICE -->
            <div id="resultCard" class="hidden mt-8 border-t border-slate-800 pt-6">
                <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                    <div class="flex-1 flex flex-col justify-between">
                        <div>
                            <span id="resSource" class="text-[10px] font-bold tracking-widest uppercase bg-indigo-950 text-indigo-300 border border-indigo-800 px-2 py-0.5 rounded-full">READY</span>
                            <h3 id="resTitle" class="text-sm font-semibold text-slate-200 line-clamp-2 mt-1.5">Link Generated Successfully</h3>
                        </div>
                    </div>
                </div>

                <!-- Explicit instruction to guide users through the Monetag Popunder -->
                <p class="mt-3 text-xs text-amber-400 bg-amber-950/40 border border-amber-900/50 px-3 py-2 rounded-lg text-center">
                    ⚠️ <b>Note:</b> First click might open a sponsor ad. Simply close it and click the button again to download!
                </p>

                <a id="resDlLink" href="" target="_blank" rel="noopener noreferrer" class="mt-4 w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold py-3.5 px-4 rounded-xl text-center block shadow-lg">
    ...
    <footer class="w-full text-center py-6 border-t border-slate-900 bg-slate-950/40 text-xs text-slate-500">
        <p>&copy; 2026 StreamGrab Network. All Rights Reserved.</p>
    </footer>
        ...
        async function processMediaLink() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showError(data.error || "All nodes are congested. Please try again.");
                }
            } catch (err) {
...
def download_video():
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if status in ["stream", "redirect"]:
                    return jsonify({
                        "success": True,
                        "title": res_data.get("title", "Decrypted Stream Ready"),
                        "download_url": res_data.get("url"),
                        "source": "PREMIUM CLOUD"
                    })
                elif status == "picker":
                    ...
                    if picker_items:
                        return jsonify({
                            "success": True,
                            "title": res_data.get("title", "Multi-Media Stream"),
    ...
    # CLEANER FALLBACK ROUTE: Redirecting to clean direct link generator instead of 9xBuddy
    clean_fallback = f"https://en.savefrom.net/1-youtube-video-downloader-4v8.html?url={video_url}"
    return jsonify({
        "success": True,
        "title": "Alternative High-Speed Mirror",
        "download_url": clean_fallback,
